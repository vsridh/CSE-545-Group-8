...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang"  ]; then
	...
	CC_SRC_PATH=github.com/hyperledger/fabric-samples/chaincode/software_security/go
	echo Vendoring Go dependencies ...
	pushd ../chaincode/software_security/go
	GO111MODULE=on go mod vendor
	popd
	echo Finished vendoring Go dependencies
elif [ "$CC_SRC_LANGUAGE" = "java" ]; then
	CC_RUNTIME_LANGUAGE=java
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/software_security/java/build/install/software_security
  echo Compiling Java code ...
  pushd ../chaincode/software_security/java
  ./gradlew installDist
  popd
  echo Finished compiling Java code
elif [ "$CC_SRC_LANGUAGE" = "javascript" ]; then
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/software_security/javascript
elif [ "$CC_SRC_LANGUAGE" = "typescript" ]; then
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples/chaincode/software_security/typescript
	echo Compiling TypeScript code into JavaScript ...
	pushd ../chaincode/software_security/typescript
	npm install
	npm run build
	popd
...
${PEER0_ORG1} lifecycle chaincode package \
  software_security.tar.gz \
  --path "$CC_SRC_PATH" \
  --lang "$CC_RUNTIME_LANGUAGE" \
  --label software_securityv1
...
REGEX='Package ID: (.*), Label: software_securityv1'
if [[ `${PEER0_ORG1} lifecycle chaincode queryinstalled` =~ $REGEX ]]; then
  PACKAGE_ID_ORG1=${BASH_REMATCH[1]}
else
  echo Could not find package ID for software_securityv1 chaincode on peer0.org1.example.com
  exit 1
fi
...
${PEER0_ORG2} lifecycle chaincode install software_security.tar.gz

echo "Installing smart contract on peer1.org2.example.com"
${PEER1_ORG2} lifecycle chaincode install software_security.tar.gz
...
if [[ `${PEER0_ORG2} lifecycle chaincode queryinstalled` =~ $REGEX ]]; then
  ...
  echo Could not find package ID for software_securityv1 chaincode on peer0.org2.example.com
  exit 1
fi
...
Next, use the software_security applications to interact with the deployed software_security contract.
The software_security applications are available in multiple programming languages.
